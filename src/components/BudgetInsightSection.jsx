                  ...
                  <div className="card insight-card glass" style={{ transition: 'all 0.3s ease' }}>
                              ...
                              <div style={{ position: 'relative', height: '180px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                              ...
                                              <div style={{ position: 'absolute', textAlign: 'center' }}>
                                                                  ...
                                                                  <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Used</div>div>
                                              </div>div>
                              </div>div>
                  
                              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginTop: '1.5rem', padding: '0 1rem' }}>
                                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                                  <span style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Total Used</span>span>
                                                                  <span style={{ fontWeight: 700, color: 'var(--text)' }}>\u20B9{stats.totalPeriodic.toLocaleString('en-IN')}</span>span>
                                              </div>div>
                                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                                  <span style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Remaining</span>span>
                                                                  <span style={{ fontWeight: 700, color: stats.remaining < 0 ? '#ef4444' : '#10b981' }}>\u20B9{Math.abs(stats.remaining).toLocaleString('en-IN')}</span>span>
                                              </div>div>
                              </div>div>
                  </div>div>
          </div>div>
...
export default BudgetInsightSection;</div>
